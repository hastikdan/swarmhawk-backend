...
# ══════════════════════════════════════════════════════════════════════════════
# Deep Scan / Pentest Request endpoints
# ══════════════════════════════════════════════════════════════════════════════

class DeepScanRequestBody(BaseModel):
    domain:        str
    contact_email: str
    company_name:  str = ""

@app.post("/deepscan/request")
def deepscan_public_request(body: DeepScanRequestBody, background_tasks: BackgroundTasks):
    """
    Public — no auth required.
    Accepts a deep-scan request from the /deepscan landing page.
    Queues a full scan + stores the request for admin review.
    """
    import re as _re
    if not _re.match(r'^([a-zA-Z0-9]([a-zA-Z0-9\-]{0,61}[a-zA-Z0-9])?\.)+[a-zA-Z]{2,}$', body.domain):
        raise HTTPException(status_code=400, detail="Invalid domain format")
    if not _re.match(r'^[^@]+@[^@]+\.[^@]+$', body.contact_email):
        raise HTTPException(status_code=400, detail="Invalid email address")

    db = get_db()

    # Prevent duplicate pending requests for same domain+email
    dup = db.table("pentest_requests")\
        .select("id")\
        .eq("domain", body.domain.lower())\
        .eq("contact_email", body.contact_email.lower())\
        .eq("status", "pending")\
        .execute()
    if dup.data:
        return {"status": "already_queued", "message": "A scan request for this domain is already pending."}

    row = db.table("pentest_requests").insert({
        "domain":        body.domain.lower(),
        "contact_email": body.contact_email.lower(),
        "company_name":  body.company_name,
        "source":        "deepscan_page",
        "status":        "pending",
    }).execute()

    request_id = row.data[0]["id"]

    # Run the enhanced scan in the background
    background_tasks.add_task(_run_deepscan_background, request_id, body.domain.lower(), body.contact_email.lower())

    return {"status": "queued", "request_id": request_id, "message": "Scan started — results will be emailed to you."}


@app.get("/admin/deepscan/requests")
def admin_list_deepscan_requests(authorization: str = Header(None)):
    """Admin — list all deep-scan requests."""
    user = get_user_from_header(authorization)
    if not is_admin(user["sub"]):
        raise HTTPException(403, "Admin only")
    db = get_db()
    rows = db.table("pentest_requests")\
        .select("*")\
        .order("created_at", desc=True)\
        .limit(200)\
        .execute()
...
@app.post("/domains/{domain_id}/deepscan")
def request_domain_deepscan(domain_id: str, background_tasks: BackgroundTasks,
                             authorization: str = Header(None)):
    """
    Authenticated — request a deep scan on a domain the user owns.
    Creates a pentest_requests record linked to the domain.
    """
    user = get_user_from_header(authorization)
    db   = get_db()
    ...
    # Require paid subscription for dashboard deep scans
    if not is_admin(user["sub"]):
        paid = db.table("purchases").select("id")\
            .eq("user_id", user["sub"])\
            .is_("cancelled_at", "null")\
        ...
        if not paid.data:
            raise HTTPException(
                status_code=403,
                detail="Deep Scan is available on the Annual plan. Upgrade to unlock."
            )

    # Get user email for results delivery
    user_row   = db.table("users").select("email").eq("id", user["sub"]).execute()
    user_email = user_row.data[0]["email"] if user_row.data else ""
    ...
    request_id = row.data[0]["id"]
    background_tasks.add_task(_run_deepscan_background, request_id, d["domain"], user_email)

    return {"status": "queued", "request_id": request_id, "message": f"Deep scan queued for {d['domain']}"}


@app.get("/domains/{domain_id}/deepscans")
def get_domain_deepscans(domain_id: str, authorization: str = Header(None)):
    """Return all completed deep-scan results for a user's domain."""
    user = get_user_from_header(authorization)
    db   = get_db()
    domain_row = db.table("domains")\
        .select("id")\
    ...
    if not domain_row.data:
        raise HTTPException(404, "Domain not found")
    rows = db.table("pentest_requests")\
        .select("id,domain,status,risk_score,critical_count,key_findings,created_at,completed_at")\
        .eq("domain_id", domain_id)\
        .order("created_at", desc=True)\
        .execute()
    return {"deepscans": rows.data or []}


def _run_deepscan_background(request_id: str, domain: str, contact_email: str):
    """
    Background task: run full 22-check scan on the domain,
    generate AI key-findings summary, update pentest_requests, email results.
    """
    db = get_db()
    try:
        db.table("pentest_requests").update({"status": "scanning"})\
            .eq("id", request_id).execute()

        if not SCANNER_AVAILABLE:
            db.table("pentest_requests").update({"status": "failed", "report_notes": "Scanner not available"})\
                .eq("id", request_id).execute()
            return
        ...
        # Generate AI key-findings summary
        key_findings = _generate_deepscan_summary(domain, result)

        db.table("pentest_requests").update({
            "status":         "completed",
            "scan_results":   result.get("checks", []),
            "risk_score":     result.get("risk_score", 0),
        ...
        print(f"[DeepScan] {domain} completed: score={result['risk_score']}, critical={result['critical']}")

        # Email results to requester
        if contact_email:
            _send_deepscan_results_email(contact_email, domain, result, key_findings)

    except Exception as e:
        import traceback
        print(f"[DeepScan] {domain} failed: {e}\n{traceback.format_exc()}")
        db.table("pentest_requests").update({
            "status":       "failed",
            "report_notes": str(e)[:500],
        }).eq("id", request_id).execute()


def _generate_deepscan_summary(domain: str, result: dict) -> str | None:
    """Generate a concise AI summary of key findings for the follow-up email."""
    AKEY = os.getenv("ANTHROPIC_API_KEY", "")
    if not AKEY:
        return None
    checks   = result.get("checks", [])
    ...
    prompt = (
        f"You are a security analyst. Summarize the top findings for {domain} "
        f"in 3-5 bullet points for a business audience. Be concrete and action-oriented.\n\n"
        f"Critical findings: {[c.get('title') for c in critical[:5]]}\n"
        f"Warnings: {[w.get('title') for w in warnings[:5]]}\n"
        f"Risk score: {result.get('risk_score', 0)}/100\n\n"
        "Format: bullet points starting with '•'. No headers. Max 150 words."
    )
    try:
        resp = _call_claude_sync(
            AKEY, "claude-haiku-4-5-20251001", 300,
            "You are a concise security briefing writer.",
            [{"role": "user", "content": prompt}],
            metadata={"feature": "deepscan_summary"},
        )
...
def _send_deepscan_results_email(email: str, domain: str, result: dict, key_findings: str | None):
    """Send deep scan results email via Resend."""
    RESEND_KEY = os.getenv("RESEND_API_KEY", "")
    if not RESEND_KEY:
        return
    score    = result.get("risk_score", 0)
    critical = result.get("critical", 0)
    findings = key_findings or f"{critical} critical finding(s) detected."
    color    = "#f87171" if score >= 70 else "#fbbf24" if score >= 40 else "#a3e635"
    html = f"""
<div style="font-family:Inter,sans-serif;background:#080810;color:#e8e8f0;padding:40px;max-width:560px;margin:0 auto;border-radius:12px">
  <div style="margin-bottom:28px">
    <span style="font-size:11px;font-weight:700;letter-spacing:3px;text-transform:uppercase;color:#a3e635">SWARMHAWK DEEP SCAN</span>
    <h2 style="font-size:24px;font-weight:800;margin:8px 0 4px;color:#e8e8f0">{domain}</h2>
    <p style="color:#8888a0;font-size:13px">Your security assessment is complete</p>
  </div>
  <div style="background:#0f0f1a;border:1px solid rgba(255,255,255,.07);border-radius:10px;padding:24px;margin-bottom:24px;text-align:center">
    <div style="font-size:56px;font-weight:900;color:{color};line-height:1">{score}</div>
    <div style="font-size:11px;color:#8888a0;text-transform:uppercase;letter-spacing:2px;margin-top:4px">Risk Score (0=best, 100=worst)</div>
  </div>
  <div style="background:#13131f;border:1px solid rgba(255,255,255,.07);border-radius:10px;padding:20px;margin-bottom:24px">
    <div style="font-size:11px;font-weight:700;color:#a3e635;text-transform:uppercase;letter-spacing:1px;margin-bottom:12px">KEY FINDINGS</div>
    <div style="font-size:14px;color:#e8e8f0;line-height:1.8;white-space:pre-line">{findings}</div>
  </div>
  <div style="margin-bottom:28px">
    <p style="font-size:14px;color:#8888a0;line-height:1.7">
      This scan used SwarmHawk's 22-check security engine. For confirmed exploit validation
      and a full penetration test report, reply to this email to discuss next steps.
    </p>
  </div>
  <a href="https://www.swarmhawk.com/deepscan" style="display:inline-block;background:#a3e635;color:#000;font-weight:700;font-size:13px;padding:12px 24px;border-radius:8px;text-decoration:none">Learn About Full Pentest →</a>
  <p style="margin-top:28px;font-size:11px;color:#4a4a60">SwarmHawk · security@swarmhawk.com · <a href="https://www.swarmhawk.com" style="color:#a3e635">www.swarmhawk.com</a></p>
</div>"""
    try:
        requests.post(
            "https://api.resend.com/emails",
            ...
            json={
                "from":    "SwarmHawk <security@swarmhawk.com>",
                "to":      [email],
                "subject": f"[SwarmHawk] Deep Scan Results: {domain} — Risk Score {score}/100",
                "html":    html,
            },
            timeout=10,
        )
    except Exception as e:
        print(f"[DeepScan] Email send failed: {e}")
